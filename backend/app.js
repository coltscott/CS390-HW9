...
app.post("/blog/create-post", (req, res) => {
  ...
  newBlog.save((err, docs) => {
    if (!err) {
      ...
      res.send("Error saving new blog post");
      console.log(
        "Error creating new record : " + JSON.stringify(err, undefined, 2)
      );
    }
  });
});
...
app.delete("/blog/:id", (req, res) => {
  ...
  Blog.findByIdAndRemove(req.params.id, (err, docs) => {
    if (!err) res.send(docs);
      ...
      console.log(
        "Error deleting a record : " + JSON.stringify(err, undefined, 2)
      );
  });
});
